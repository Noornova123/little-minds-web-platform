...
import { Brain, LogOut, Menu, X, Home, Library, FileText, User, CalendarDays, LifeBuoy, ChevronLeft, School, ClipboardList, MessageSquarePlus, Stethoscope } from 'lucide-react';
...
export function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const onDiagnostics = path.startsWith('/dashboard/diagnostics');
          ...
          <div ref={menuRef} className="relative">
            ...
            {menuOpen && (
              <div className="absolute left-0 top-full mt-2 w-60 lm-card p-2 lm-fade-up z-40">
                ...
                <MenuItem icon={<Stethoscope size={18} />} label="Diagnostics" active={onDiagnostics} onClick={() => navigate('/dashboard/diagnostics')} />
...
function showBack(path: string): boolean {
  ...
  if (path.startsWith('/dashboard/diagnostics')) return true;
