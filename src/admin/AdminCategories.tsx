...
export function AdminCategories() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<LibraryCategory | null>(null);

  async function load() {
    setLoading(true);
    const { data } = await supabase.from('library_categories').select('*').order('sort_order', { ascending: true });
    setCats((data as LibraryCategory[]) ?? []);
    setLoading(false);
  }
  ...
  async function add(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('library_categories').insert({ name: newName.trim(), sort_order: nextOrder });
    setBusy(false);
  ...
  async function rename(id: string, name: string) {
    await supabase.from('library_categories').update({ name }).eq('id', id);
    load();
  }

  async function move(c: LibraryCategory, dir: -1 | 1) {
    ...
    await Promise.all([
      supabase.from('library_categories').update({ sort_order: other.sort_order }).eq('id', c.id),
      supabase.from('library_categories').update({ sort_order: c.sort_order }).eq('id', other.id),
    ]);
    load();
  }
  ...
  async function doDelete() {
    ...
    await supabase.from('library_categories').delete().eq('id', confirmDelete.id);
    ...
    <div className="space-y-5 lm-fade-up">
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-1">
          <Tag size={18} className="text-[var(--terracotta)]" />
          <h1 className="text-xl font-extrabold text-[var(--ink)]" style={{ fontFamily: 'Fraunces, serif' }}>Library categories</h1>
        </div>
        <p className="text-sm text-[var(--ink-soft)] mb-4">These labels organize the Content Library teachers browse. Add or rename any time.</p>
        ...
        {loading ? <Spinner label="Loading…" /> : cats.length === 0 ? (
          <EmptyState title="No categories yet" hint="Add one to start organizing the library." />
        ) : (
      ...
      <ConfirmDialog
        ...
        message={`Delete "${confirmDelete?.name}"? Library activities using this label won't be deleted, but they'll need re-categorizing.`}
        confirmLabel="Delete"
        danger
      />
    </div>
